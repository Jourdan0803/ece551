...
ss_monthly_t parseLine(char * line) {
  ...
  //catch the year,month and num
  if (sscanf(line, "%4d-%2d,%lf%99s", &year, &month, &num,remaining) != 3) {
...
int main() {
    char * line = {"9999-09,3.99999"};
    ss_monthly_t x = parseLine(line);
    printf("year:%d,month:%d,num:%f",x.year,x.month,x.num);
    return 0;
}